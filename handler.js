   ...
   try {
      ...
      if (m.chat.endsWith('broadcast') && m.mtype != 'protocolMessage') {
         ...
         if (/video|image/.test(m.mtype)) {
            caption += `${body ? body : ''}\n\n`
            caption += `*Dari : @${m.sender.replace(/@.+/, '')} (${client.getName(m.sender)})*`
      ...
      if ((body && body.length != 1 && (isPrefix = (myPrefix || '')[0])) || body && univers.includes((body.split` ` [0]).toLowerCase())) {
         ...
         for (let name in is_commands) {
            ...
            if (!m.isGroup && !['owner', 'create_bot'].includes(name) && chats && !isPrem && !users.banned && setting.groupmode) return client.sendMessageModify(m.chat, ` Untuk bisa menggunakan bot di personal chat (PC) kamu harus mempunyai limits minimal *5000,M* atau cara cepat dengan upgrade ke premium user.\n\nJika ingin upgrade ke premium silahkan masuk ke grup ini dan kirim *${prefixes[0]}owner*`, m, {
               largeThumb: true,
               thumbnail: await Func.fetchBuffer('https://telegra.ph/file/5b41c6d53b226403e8c9a.jpg'),
               url: 'https://chat.whatsapp.com/FmobuVh6GwM1JtZVAx7US9'
            ...
            if (cmd.limit && users.limit < 1) {
               return client.reply(m.chat, `Penggunaan bot Anda telah mencapai batas dan akan disetel ulang pada pukul 00.00\n\nUntuk mendapatkan lebih banyak batas, tingkatkan ke paket premium ketik *${prefixes[0]}premium*`, m).then(() => users.premium = false)
               continue
            }
         ...
         for (let name in is_events) {
            ...
            if (!m.isGroup && chats && !isPrem && !users.banned && setting.groupmode) return client.sendMessageModify(m.chat, `Untuk bisa menggunakan bot di personal chat (PC) kamu harus mempunyai limits minimal *5000,M* atau cara cepat dengan upgrade ke premium user.\n\nJika ingin upgrade ke premium silahkan masuk ke grup ini dan kirim *${prefixes[0]}owner*`, m, {
               largeThumb: true,
               thumbnail: await Func.fetchBuffer('https://telegra.ph/file/5b41c6d53b226403e8c9a.jpg'),
               url: 'https://chat.whatsapp.com/FmobuVh6GwM1JtZVAx7US9'
            }).then(() => chats.lastchat = new Date() * 1)
